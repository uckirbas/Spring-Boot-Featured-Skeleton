...
interface DistrictRepo : JpaRepository<District, Long> {
    ...
    @Query("SELECT d FROM District d WHERE (:q IS NULL OR (d.nameEn LIKE %:q% OR d.nameBn LIKE %:q%)) AND (:div IS NULL OR d.division=:div)")
    fun search(@Param("q") query: String, @Param("div") division: Division?): List<District>
}
