...
interface UnionRepo : JpaRepository<Union, Long> {
    ...
    @Query("SELECT u FROM Union u WHERE (:q IS NULL OR (u.nameEn LIKE %:q% OR u.nameBn LIKE %:q%)) AND (:upazila IS NULL OR u.upazila=:upazila)")
    fun search(@Param("q") query: String, @Param("upazila") upazila: Upazila?): List<Union>
}
